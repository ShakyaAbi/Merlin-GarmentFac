...
export function FinishedGoodCsvActions({ filters = {}, title = 'Articles CSV', onSuccess }: Props) {
  ...
  const handleExport = async () => {
    ...
    setError(null)
    try {
      const blob = await finishedGoodApi.exportCSV(filters)
      downloadBlob(blob, `articles_${new Date().toISOString().split('T')[0]}.csv`)
      setShowExport(false)
    } catch (err: any) {
  ...
  const handleTemplate = async () => {
    ...
    setError(null)
    try {
      const blob = await finishedGoodApi.downloadImportTemplate()
  ...
  const handleImport = async () => {
    ...
    setError(null)
    try {
      await finishedGoodApi.uploadCSV(file)
      setShowImport(false)
      setFile(null)
      onSuccess?.()
    } catch (err: any) {
  ...
  return (
    <>
      ...
      <Modal isOpen={showImport} onClose={() => setShowImport(false)} title={`Import ${title}`} size="md">
        <div className="space-y-4">
          <div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-600">
            Download the template, fill it with article records including the article code, then upload the CSV.
          </div>
      ...
      <Modal isOpen={showExport} onClose={() => setShowExport(false)} title={`Export ${title}`} size="md">
        <div className="space-y-4">
          <div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-600">
            Export the current article catalog or the selected article set as CSV.
          </div>
