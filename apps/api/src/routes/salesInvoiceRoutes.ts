...
router.patch('/:id/payments/:paymentId', authenticate, requireRoles(Role.ADMIN, Role.MANAGER, Role.DATA_ENTRY), validate({ body: updateSalesInvoicePaymentSchema }), salesInvoices.updatePayment)
router.delete('/:id/payments/:paymentId', authenticate, requireRoles(Role.ADMIN, Role.MANAGER, Role.DATA_ENTRY), salesInvoices.removePayment)
