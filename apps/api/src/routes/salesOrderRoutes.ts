...
router.patch('/:id', authenticate, requireRoles(Role.ADMIN, Role.MANAGER), validate({ body: updateSalesOrderSchema }), salesOrders.update)
router.post('/:id/confirm', authenticate, requireRoles(Role.ADMIN, Role.MANAGER), salesOrders.confirm)
...
router.post('/:id/invoice', authenticate, requireRoles(Role.ADMIN, Role.MANAGER), salesOrders.invoice)
